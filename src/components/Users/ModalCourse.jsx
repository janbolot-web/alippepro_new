...
const ModalCourse = ({ setShowModal, userId, userCourses }) => {
  ...
  for (let i = 0; i < lessonIds?.length; i++) {
    for (let j = 0; j < modulesData?.length; j++) {
      if (lessonIds[i]?._id === modulesData[j]?._id) {
        modulesDataUser.push(modulesData[j]);
      }
    }
  }
console.log('idCourse',idCourse);
  ...
  console.log('!!!!!!!',modulesDataUser);
  return (
    <div className="modal">
      <div className="modal__container container">
        <div className="modal__close" onClick={closeBtn}>
          <MdOutlineClose color="tomato" size={24} />
        </div>
        <div className="modal__header">
          <input
            ...
            className="modal__idCourse"
          />
          <div className="modal__searchBtn">
            <FiSearch size={22} color="#0099f8" onClick={searchModules} />
          </div>
        </div>
        <ul className="modal__list">
          {modules &&
            modulesDataUser?.map((module, i) =>
              module?.isAccess === false ? (
                <li
                  key={module._id}
                  onClick={() => console.log(module._id)}
                  className="modal__item"
                >
                  <p>{module.name}</p>{" "}
                  <div
                    className="modal__banBtn"
                ...
                >
                  <p>{module?.name}</p>{" "}
                  <div
                    className="modal__lockBtn"
                    onClick={() => onAddModuleToUser({ moduleId: module._id })}
                  >
                    Открыть доступ
                  </div>
                </li>
